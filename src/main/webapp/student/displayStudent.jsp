...
<%-- => [변경]를 클릭한 경우 학생정보 삭제 입력페이지(updateFormStudent.jsp)로 이동 - 학생번호 전달 --%>
<%
...
<table align="center" border="1" cellspacing="0" cellpadding="1" width="800">
	...
	<% if(studentList.isEmpty()) { //List 인스턴스의 요소가 없는 경우%>
		<tr align="center">
			<td colspan="7">검색된 학생정보가 없습니다.</td>
		</tr>
	<% } else {//List 인스턴스에 저장된 요소가 없는 경우 %>
		<%-- List 인스턴스에 저장된 요소를 차례대로 제공받아 반복처리 --%>
...
<p align="center" style="color: tomato;"><%=message %></p>

<script type="text/javascript">
function removeStudent(no) {
	if(confirm("학번 '" + no + "' 학생정보를 삭제 하시겠습니까?")) {
